...
int inv_imu_set_accel_fsr(inv_imu_device_t *s, accel_config0_ap_accel_fs_sel_t accel_fsr)
{
	int             status = INV_IMU_OK;
	accel_config0_t accel_config0;
...
int inv_imu_set_gyro_fsr(inv_imu_device_t *s, gyro_config0_ap_gyro_fs_sel_t gyro_fsr)
{
	int            status = INV_IMU_OK;
	gyro_config0_t gyro_config0;
...
int inv_imu_get_register_data(inv_imu_device_t *s, inv_imu_sensor_data_t *data)
{
	...
	status |= inv_imu_read_reg(s, ACCEL_DATA_X_0, sizeof(inv_imu_sensor_data_t), (uint8_t *)data);
